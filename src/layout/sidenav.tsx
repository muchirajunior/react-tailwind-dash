      ...
      <span className='h-5'></span>
      {
        routes.map((route) =>
          <Link
            to={route.path}
            key={route.path}
            className={`flex flex-row align-middle rounded m-1 bg-gray-100 hover:bg-blue-600 hover:text-white p-2 font-bold text-lg ${route.path===current ? "bg-blue-700 text-white":null}`}
          >
